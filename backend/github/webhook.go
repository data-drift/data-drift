...
func processWebhookInTheBackground(config common.Config, c *gin.Context, InstallationId int, client *github.Client, ownerName string, repoName string) bool {
	...
	for _, metric := range config.Metrics {
		...
		if metadataChartError != nil {
			...
			reports.CreateSummaryReport(common.SyncConfig{NotionAPIKey: config.NotionAPIToken, NotionDatabaseID: config.NotionDatabaseID}, metric, metadataChartResults, string(InstallationId))
		}
	}
	return false
}
